# Claude Code Project Instructions — inventory-kotlin

## Project Overview

This is **inventory-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`inventory`](https://crates.io/crates/inventory).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/inventory/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/inventory>
