...
// of the pipeline:
//
//   1. `embedSwiftExportForXcode` produced `Inventory.swiftmodule/`
...
//      `import Inventory` above would fail at compile time.
//
//   2. The static archive `libInventory.a` (produced by the
...
//      architecture arm64".
//
//   3. The Kotlin `swiftExport { moduleName = "Inventory" }` and
//      `flattenPackage = "io.github.kotlinmania.inventory"` configuration in
...
//      via the `InventoryLibrary` product.
//
...
final class InventoryExportTests: XCTestCase {
    func testSwiftModuleLoads() throws {
        XCTAssertTrue(true, "Inventory swift module imported cleanly")
    }
}
