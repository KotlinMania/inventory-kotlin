...
Kotlin Multiplatform line-by-line clean-room port of the Rust crate [`inventory`](https://crates.io/crates/inventory).

> **Status: scaffold — porting has not started.** This repo currently contains build infrastructure only. The upstream Rust source for the `inventory` crate will be cloned into `tmp/` (gitignored) when porting begins.

## About

Typed compile-time plugin registry
...
Original work copyrighted by the upstream `inventory` authors. Kotlin port copyright (c) 2026 Sydney Renee and The Solace Project.
